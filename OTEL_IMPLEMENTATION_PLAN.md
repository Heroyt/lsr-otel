...
| `lsr/caching` | `Cache::load()` / `bulkLoad()` and generator fallback | Hit/miss/load counters, operation duration, generator failure | Instrument high-level cache operations to avoid duplicate Redis spans. Use per-operation/per-request instruments; current static cumulative counters are unsafe labels for RR request metrics. |
